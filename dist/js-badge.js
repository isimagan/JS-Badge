...
// src/constants.js
var JS_BADGE_TYPE = "js-badge";
var JS_BADGE_NAME = "JS Badge";
var JS_BADGE_VERSION = version_default.version;
...
${match[1]}`
    ...
    console.error("[js-badge] Template error:", error, value);
...
var BADGE_STYLES = `
  ...
  .badge {
    --badge-color: var(--js-badge-icon-color);
    ...
    background: var(--js-badge-background-color);
    width: auto;
  ...
  .label {
    ...
    color: var(--js-badge-secondary-color);
    line-height: 10px;
    max-width: 100%;
    overflow: hidden;
  ...
  .content {
    ...
    letter-spacing: 0.1px;
    color: var(--js-badge-primary-color);
    max-width: 100%;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
...
async function runServiceAction(hass, actionConfig) {
  ...
  if (!serviceName || !hass?.callService) {
    console.warn("[js-badge] Invalid service action:", actionConfig);
    return;
  }
  const separator = serviceName.indexOf(".");
  if (separator < 1 || separator === serviceName.length - 1) {
    console.warn("[js-badge] Invalid service name:", serviceName);
    return;
  }
...
async function runMultiAction(host, hass, baseConfig, actionConfig) {
  ...
  if (!Array.isArray(sequence)) {
    console.warn("[js-badge] Multi-action sequence must be an array.");
    return;
  }
...
var BadgeInteractions = class {
  ...
  run(action) {
    void runConfiguredAction(
      ...
      console.error(`[js-badge] ${action} action failed:`, error);
    });
  }
  onPointerDown() {
...
// src/js-badge.js
var JsBadge = class extends HTMLElement {
  constructor() {
    super();
  ...
  setConfig(config) {
    if (!config || typeof config !== "object") {
      throw new Error("JS Badge requires a configuration object.");
    }
    this._config = config;
    this._render();
  }
...
setTimeout(registerJsBadge, 1e3);
console.info(
