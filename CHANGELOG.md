...
- Published the project as JS Badge.

### Changed

- Renamed the HACS package and generated bundle to `js-badge`.
- Renamed the Home Assistant badge type to `custom:js-badge`.
- Updated product names, internal identifiers, CSS variables and documentation for JS Badge.

### Breaking changes

- Existing configurations must change `type: custom:custom-js-badge` to `type: custom:js-badge`.
...
- The badge type remains `custom:js-badge`.
- Existing action configurations and JavaScript templates remain supported.
- HACS continues to install the generated `dist/js-badge.js` bundle.
...
- Reduced `js-badge.js` to the custom element lifecycle and coordination logic.
- Rebuilt the generated `dist/js-badge.js` bundle from the modular source.
...
- Existing `custom:js-badge` configurations were not affected.
...
- Removed the separate generated `dist/js-badge.js`, `dist/custom-template-badge.js` and `dist/custom-template-badge-editor.js` files.

### Changed

- The project now contains only `custom:js-badge`.
- Split the editable JavaScript source into modules under `src/`.
- Added an esbuild-based build process.
- HACS now installs one generated bundle: `dist/js-badge.js`.
- Rewrote the README for the JavaScript badge only.
- Kept the existing `custom:js-badge` type to avoid breaking JavaScript badge configurations.
...
- Added `js-badge.js` as the shared HACS entrypoint.
